...
| 1–8 | 已翻开数字（周围雷数） | 翻开 |
| 9 | 问号按下态 | 按住问号格 |
| 10 | 终局显示的雷 | 原版失败后未标旗的雷；本项目新逻辑不再使用 |
| 11 | 终局显示的错旗（×） | 连开踩雷时邻居中标错的旗 |
| 12 | 踩中的雷（红底爆炸） | 左键踩中的雷；连开踩雷时邻居中未标旗的雷（高亮） |
| 13 | 问号（?） | 右键循环第二态 |
| 14 | 旗 | 右键标旗 |
| 15 | 未翻开（凸起） | 初始状态 |
| 16 | 边框 | 棋盘外圈（不绘制） |
...
- **终局转换（失败显示为有意差异，见 §19）**：胜利 → 所有雷变为旗（14），且剩余雷数显示归 0。
- **失败终局显示（UI 层叠加渲染，Core 棋盘状态不变）**：
  - 左键踩雷（REVEAL）：只高亮踩中的雷（12 红底），其余格子保持原样。
  - 连开踩雷（CHORD）：显示连开点全部 8 邻居（Core 不额外翻开，UI 直接读取棋盘预计算状态渲染）——非雷格正常显示数字/空格；邻居中未标旗的雷全部高亮（12，多个同时高亮）；标对的旗（雷上标旗）保留旗帜外观（14），不高亮；标错的旗（非雷却标旗）显示红叉（11）。
...
class Cell:
    ...
    adjacent_mines: int  # 3×3 邻域雷数（布雷后预计算）
```
...
首次点击（Game_FirstClick）：
  ...
  3. 遍历棋盘，为每个格子预计算 3×3 邻域雷数（写入 adjacent_mines）
  4. 启动计时，正常执行翻开/洪水填充
```
...
def open_cell(board, row, col) -> None:
    """翻开单格；adjacent_mines 已在布雷后预计算（§6.3），这里只读取。"""
    v = board[row][col]
    if v.is_revealed:
        return
...
动作：逐个处理 8 邻居，遇到雷立即中断（不再翻开其余格子）
    - 邻居是旗 → 跳过（不翻开）
    - 邻居非雷 → flood_fill
    - 邻居是雷 → 立即返回 lost，标记 reason=CHORD
结果：lost → Game_End(LOSE, reason=CHORD, coord=连开点)；否则若 已翻开==需翻开 → Game_End(WIN)

> 连开踩雷时 Core 不继续翻开其余邻居；终局 UI 直接读取 neighbors8(coord) 的预计算状态，按 §4.5 完成"显示全部邻居"的叠加渲染。
```

### 6.6 胜负判定

- 胜利：**所有非雷格全部翻开**即胜（`已翻开非雷格数 == 宽×高 − 雷数`）；**雷是否全部标旗不影响胜利**。
- 失败：左键翻开雷（reason=REVEAL）、或 chord 踩雷（reason=CHORD）；失败后的终局显示见 §4.5。
...
| `GameWon` | — | 胜利重绘 + 音效 + 对话框 |
| `GameLost` | (reason, coord) | 按 §4.5 失败终局叠加渲染 + 音效 + 对话框 |
...
> `GameLost.reason` 取值 `REVEAL`（左键踩雷）/ `CHORD`（连开踩雷），`coord` 为触发坐标（踩中的雷或连开点）。
...
终局失败叠加渲染（§4.5）同样由 Renderer 依据 `GameLost(reason, coord)` 完成，不修改 Core 棋盘。
...
- 终局叠加渲染：收到 `GameLost(reason, coord)` 后按 §4.5 绘制——REVEAL 只高亮 coord 的雷（12）；CHORD 高亮 neighbors8(coord) 中未标旗的雷（12）、标对的旗保留旗帜（14）、标错的旗画红叉（11）、非雷格正常显示。
...
- Reveal：单格、0 区扩散、角落/边界、重复翻开、旗不翻开；邻域雷数预计算正确（每格 adjacent_mines == 实际 3×3 雷数）。
...
按 §4.5 行为细节逐项验证（左键/右键/旗/问号/chord/重启/计时/胜负）；失败终局显示按原因（左键只高亮该雷 / 连开显示邻居并高亮雷、错旗红叉）逐项对照。
...
| 失败终局显示 | 失败后整盘显示所有未标雷与所有错旗 | 左键踩雷只高亮该雷；连开踩雷只显示连开点邻居（未标旗雷高亮、标对旗保留、错旗红叉） | 聚焦失误点，减少信息噪音 |

...
- **PySide6 尚未安装**：`pip install PySide6>=6.10`；若 cp314 轮子缺失，改用 Python 3.13 环境。
...
def open_cell(board, row, col) -> None:
    """翻开单格；0 雷格入 BFS 队列。"""
    v = board[row][col]
    if v.is_revealed: return
    if v.mark == Mark.FLAG or is_border(v): return   # 旗/边框不翻开
    v.is_revealed = True
...
def flood_fill(board, row, col) -> None:
    ...
    _queue.clear(); _queue.append((row, col)); open_cell(row, col)
    while _queue:
        r, c = _queue.popleft()
...
| 阶段 1 工程骨架 | pyproject、venv、PySide6/pytest/mypy/ruff、包结构、空窗口 | ruff/mypy/手动 pytest 通过，空窗口启动 |
